...
from core.services.lessons_service import LessonsService, TimeState
from core.services.time_service import TimeService
from models.component_settings.schedule_component_settings import (
...
class ScheduleComponent(QWidget):
    ...
    def __init__(
        ...
        if self.lessons_service:
            self.lessons_service.post_main_timer_ticked.connect(
                self.on_post_main_timer_ticked
            )
            self.lessons_service.current_time_state_changed.connect(
                self.on_current_time_state_changed
            )
            # 初始化状态
    ...
    def init_ui(self):
        ...
        placeholder_container.setAttribute(
    ...
    def update_content(self):
        ...
        try:
            ...
            if self.exact_time_service:
                current_date = (
                    self.exact_time_service.get_current_local_datetime().date()
                )
            else:
            ...
            self.tomorrow_class_plan = self.get_class_plan_by_date(
    ...
    def update_tomorrow_label(self):
        """更新明天标签显示状态"""
        show_tomorrow = False
        tomorrow_schedule_empty = (
            not self.tomorrow_class_plan or not self.tomorrow_class_plan.get("lessons")
        )
        today_schedule_empty = (
            not self.current_class_plan or not self.current_class_plan.get("lessons")
        )
    ...
    def update_empty_placeholder(self):
        ...
        if not target_class_plan or not target_class_plan.get("lessons"):
            if self.settings.show_placeholder_on_empty_class_plan:
                if self.is_after_school:
                    self.empty_placeholder.setText(
                        self.settings.placeholder_text_all_class_ended
                    )
                else:
                    self.empty_placeholder.setText(
                        self.settings.placeholder_text_no_class
                    )
                self.empty_placeholder.setVisible(True)
    ...
    def sync_with_lessons_service(self):
        ...
        # 同步状态变量
        self.is_after_school = (
            self.lessons_service.current_state == TimeState.AFTER_SCHOOL
            or self.lessons_service.current_class_plan is None
        )
        ...
        self.show_current_lesson_only_on_class = getattr(
            self.lessons_service, "show_current_lesson_only_on_class", False
        )

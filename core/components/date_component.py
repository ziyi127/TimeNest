...
class DateComponent(QWidget):
    ...
    def __init__(self, lessons_service=None, exact_time_service: TimeService = None, settings_service=None):
        ...
        if self.lessons_service:
            ...
            self.timer.start(10000)  # 10秒检查一次，提高响应性
    ...
    def init_ui(self):
        ...
        font = QFont("Microsoft YaHei", 14, QFont.Normal)
...
<<<<<<< SEARCH
    ...
    def update_date(self):
        ...
        try:
            # 输入验证
            if not isinstance(self.settings, type(self.settings)):
                raise ValueError("无效的设置对象")
            ...
            try:
                ...
                # 验证结果安全性
                if not isinstance(formatted_date, str) or len(formatted_date) > 100:
                    raise ValueError("日期格式化结果异常")
...
<<<<<<< SEARCH
            # 出错时显示更详细的错误信息
            self.date_display.setText(f"日期错误: {str(e)}")
...
<<<<<<< SEARCH
    def update_date(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"日期格式化错误: {format_error}")
                self.date_display.setText("日期格式错误")
            
        except Exception as e:
            self.logger.error(f"更新日期组件内容时出错: {e}", exc_info=True)
            # 出错时显示更详细的错误信息
            self.date_display.setText(f"日期错误: {str(e)}")
    ...
    def update_date(self):
        ...
        # 更新显示
        old_text = self.date_display.text()
        self.date_display.setText(formatted_date)
        
        # 如果日期发生变化，触发动画
        if old_text != formatted_date and self.fade_animation:

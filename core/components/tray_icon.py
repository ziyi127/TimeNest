...
# 全局托盘图标实例
_tray_icon = None  # pylint: disable=invalid-name
...
class TrayIcon(QSystemTrayIcon):
    ...
    # 插件接口信号
    plugin_menu_item_added = Signal(str, object)  # 菜单项名称, 回调函数
    plugin_menu_item_removed = Signal(str)  # 菜单项名称

    def __init__(self, parent: QWidget | None = None):
        super().__init__(parent)
        self.logger = logging.getLogger(__name__)
        self.is_main_window_visible = True
        self.plugin_menu_items = {}  # 存储插件菜单项 {name: (callback, action)}
        self.init_tray()
    ...
    def setup_icon(self):
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                self.setIcon(
                    QIcon.fromTheme(
                        "application-x-executable", QIcon.fromTheme("application")
                    )
                )
            ...
            self.logger.warning("设置托盘图标时出错: %s", e)
            # 使用默认图标
            self.setIcon(self.get_default_icon())
    ...
    def create_context_menu(self):
        ...
        # 插件菜单项分隔符
        self.plugin_menu_separator = menu.addSeparator()
        ...
        dev_tools_action.triggered.connect(
            lambda: self.show_message("调试", "DevTools功能")
        )
        debug_menu.addAction(dev_tools_action)

        dev_portal_action = QAction("DevPortal", debug_menu)
        dev_portal_action.triggered.connect(
            lambda: self.show_message("调试", "DevPortal功能")
        )
        ...
        enable_temp_plan_action.triggered.connect(
            lambda: self.show_message("调试", "启用临时课表功能")
        )
        ...
        # 连接插件接口信号
        self.plugin_menu_item_added.connect(self._add_plugin_menu_item)
        self.plugin_menu_item_removed.connect(self._remove_plugin_menu_item)
    ...
    def _add_plugin_menu_item(self, name: str, callback):
        """添加插件菜单项"""
        # 检查是否已存在同名菜单项
        if name in self.plugin_menu_items:
            self.logger.warning("插件菜单项 '%s' 已存在，将被替换", name)
            # 移除旧的菜单项
            old_action = self.plugin_menu_items[name][1]
            self.contextMenu().removeAction(old_action)

        # 创建新的菜单项
        action = QAction(name, self.contextMenu())
        action.triggered.connect(callback)
        self.plugin_menu_items[name] = (callback, action)

        # 将菜单项添加到菜单中，放在分隔符之后
        self.contextMenu().insertAction(self.plugin_menu_separator, action)
        self.logger.info("已添加插件菜单项: %s", name)

    def _remove_plugin_menu_item(self, name: str):
        """移除插件菜单项"""
        if name in self.plugin_menu_items:
            callback, action = self.plugin_menu_items[name]
            self.contextMenu().removeAction(action)
            del self.plugin_menu_items[name]
            self.logger.info("已移除插件菜单项: %s", name)
        else:
            self.logger.warning("插件菜单项 '%s' 不存在", name)
...
"""
    ...
    def _open_settings(self):
        """打开设置"""
        self.logger.info("点击托盘菜单'应用设置'项")
        self.open_settings.emit()

    def _open_profiles(self):
        """打开档案"""
        self.logger.info("点击托盘菜单'编辑档案'项")
        self.open_profiles.emit()

    def _load_temp_class_plan(self):
        """加载临时课表"""
        self.logger.info("点击托盘菜单'加载临时课表'项")
        self.load_temp_class_plan.emit()

    def _swap_classes(self):
        """换课"""
        self.logger.info("点击托盘菜单'换课'项")

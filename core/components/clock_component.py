...
class ClockComponent(QWidget):
    ...
    def __init__(self, lessons_service: Optional[Any] = None, exact_time_service: Optional[Any] = None, settings_service: Optional[Any] = None):
        super().__init__()
        ...
        # 设置组件
        self.settings: ClockComponentSettings = ClockComponentSettings()
        
        # 动画相关
        self.separator_animation: Optional[QPropertyAnimation] = None
    ...
    def init_ui(self):
        ...
        self.main_time_display.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignLeft)
    ...
    def update_content(self):
        ...
        try:
            ...
            # 根据设置决定时间格式
            show_seconds: bool = bool(self.settings.show_seconds)
            if show_seconds:
                time_format = "%H:%M:%S"
            else:
                time_format = "%H:%M"
    ...
    def update_separator_visibility(self):
        ...
        # 当显示秒数时，分隔符应该隐藏
        flash_time_separator: bool = bool(self.settings.flash_time_separator)
        show_seconds: bool = bool(self.settings.show_seconds)
        if not flash_time_separator or show_seconds:
            self.is_time_separator_showing = True
        else:
        ...
        # 添加动画效果到分隔符
        if flash_time_separator and not show_seconds:
            self.animate_separator()
    
    def animate_separator(self):
        """为时间分隔符添加动画效果"""
        # 停止正在运行的动画
        if self.separator_animation and self.separator_animation.state() == QPropertyAnimation.State.Running:

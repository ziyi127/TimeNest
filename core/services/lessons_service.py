...
class LessonsService(QObject):
    ...
    def __init__(
        ...
        profile_service: Optional[object] = None,
        ...
        # 状态变量
        self.current_class_plan: Optional[Dict[str, Any]] = None
        self.current_selected_index = -1
        self.next_class_subject: Optional[Dict[str, Any]] = None
        self.next_breaking_time_layout_item: Optional[Dict[str, Any]] = None
        self.on_class_left_time = timedelta()
        self.current_state = TimeState.NONE
        self.current_overlay_status = TimeState.NONE
        self.current_time_layout_item: Optional[Dict[str, Any]] = None
        self.current_subject: Optional[Dict[str, Any]] = None
        ...
        self.next_class_time_layout_item: Optional[Dict[str, Any]] = None
    ...
    def stop_main_timer(self) -> None:
        """停止主定时器"""
        self.main_timer.stop()

    def main_timer_on_tick(self) -> None:
        """主定时器触发事件"""
        try:
            # 发出预处理信号
    ...
    def process_lessons(self) -> None:
        ...
        next_class_subject: Optional[Dict[str, Any]] = None
        current_time_layout_item: Optional[Dict[str, Any]] = None
        next_class_time_layout_item: Optional[Dict[str, Any]] = None
        next_breaking_time_layout_item: Optional[Dict[str, Any]] = None
        on_class_left_time: Optional[timedelta] = None
        on_breaking_time_left_time: Optional[timedelta] = None
        is_lesson_confirmed: Optional[bool] = None
        is_class_plan_loaded: Optional[bool] = None
        ...
        if not goto_final:
            ...
            if current_time_layout_item:
                current_selected_index = (
                    layout.index(current_time_layout_item)
                    if current_time_layout_item in layout
                    else -1
                )
                ...
                if time_type == 0:  # 上课
                    ...
                    if (
                        class_index >= 0
                        and self.current_class_plan
                        and "classes" in self.current_class_plan
                        and len(self.current_class_plan["classes"]) > class_index
                    ):
                    ...
                    current_subject = {
                        "name": current_time_layout_item.get(
            ...
            if next_class_time_layout_item:
                class_index = self.get_class_index(
                    layout.index(next_class_time_layout_item)
                )
                if (
                    class_index >= 0
                    and self.current_class_plan
            ...
            if current_state == TimeState.ON_CLASS and next_breaking_time_layout_item:
                next_break_start = self.parse_time_string(
                    next_breaking_time_layout_item.get("start_time", "00:00")
                )
                on_breaking_time_left_time = datetime.combine(
                    datetime.today(), next_break_start
                ) - datetime.combine(datetime.today(), now)
            elif next_class_time_layout_item:
                next_class_start = self.parse_time_string(
                    next_class_time_layout_item.get("start_time", "00:00")
                )
                on_class_left_time = datetime.combine(
                    datetime.today(), next_class_start
                ) - datetime.combine(datetime.today(), now)
        ...
        self.current_selected_index = (
            current_selected_index if current_selected_index is not None else -1
        )
        self.current_state = (
            current_state if current_state is not None else TimeState.NONE
        )
        ...
        self.on_class_left_time = (
            self.at_least_zero(on_class_left_time)
            if on_class_left_time is not None
            else timedelta()
        )
        self.on_breaking_time_left_time = (
            self.at_least_zero(on_breaking_time_left_time)
            if on_breaking_time_left_time is not None
            else timedelta()
        )
        self.is_lesson_confirmed = (
            is_lesson_confirmed if is_lesson_confirmed is not None else False
        )
        self.is_class_plan_loaded = (
            is_class_plan_loaded if is_class_plan_loaded is not None else False
        )
    ...
    def get_class_index(self, index: int) -> int:
        ...
        if (
            index < 0
            or not self.current_class_plan
            or "time_layout" not in self.current_class_plan
            or "layouts" not in self.current_class_plan["time_layout"]
        ):
            return -1
        ...
        # 获取所有上课时间点
        class_time_points = [
            layout for layout in layouts if layout.get("time_type", 0) == 0
        ]
    ...
    def load_current_class_plan(self) -> None:
        """加载当前课程表"""
    ...
    def get_valid_time_layout_items(self) -> List[Dict[str, Any]]:
        ...
        valid_items = [
            item
            for item in time_layout["layouts"]
    ...
    def debug_trigger_on_class(self) -> None:
        """调试触发上课事件"""
        self.on_class.emit()

    def debug_trigger_on_breaking_time(self) -> None:
        """调试触发课间事件"""
        self.on_breaking_time.emit()

    def debug_trigger_on_after_school(self) -> None:
        """调试触发放学事件"""
        self.on_after_school.emit()

    def debug_trigger_on_state_changed(self) -> None:
        """调试触发状态变化事件"""
        self.current_time_state_changed.emit()

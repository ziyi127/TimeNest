...
class TimeService(QObject):
        ...
        for attempt in range(self.max_retry_attempts):
            for server in servers_to_try:
                try:
                    ...
                    last_exception = e
                    self.logger.warning(
                        f"从服务器 {server} 同步时间失败 (尝试 {attempt + 1}/{self.max_retry_attempts}): {e}"
                    )
                    continue  # 尝试下一个服务器
            ...
            if attempt < self.max_retry_attempts - 1:
                self.logger.info(
                    f"所有服务器同步失败，等待5秒后进行第 {attempt + 2} 次重试"
                )
                time.sleep(5)
    ...
    def get_current_local_datetime(self) -> datetime:
        ...
        base_time = (
            base_time
            if base_time > self.prev_datetime or not self.need_waiting
            else self.prev_datetime
        )
    ...
    def enable_exact_time(self, enabled: bool) -> None:
        """启用或禁用精确时间"""
    ...
    def set_exact_time_server(self, server: str) -> None:
        """设置精确时间服务器"""

...
class TimeLayoutItem:
    ...
    def time_type_text(self) -> str:
        """获取时间类型文本"""
        type_map = {0: "上课", 1: "课间休息", 2: "分割线", 3: "行动"}
...
class TimeLayout:
    ...
    def insert_time_point(self, index: int, item: TimeLayoutItem) -> None:
        """在指定索引处插入时间点"""
        self.layouts.insert(index, item)
        self.updated_at = datetime.now()

    def remove_time_point(self, item: TimeLayoutItem) -> None:
        """删除指定的时间点"""
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TimeLayout":
        ...
        layout.layouts = [
            TimeLayoutItem.from_dict(item_data) for item_data in layouts_data
        ]

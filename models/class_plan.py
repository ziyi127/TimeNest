...
class ClassPlanGroup:
    ...
    def create_default_group(cls) -> "ClassPlanGroup":
        """创建默认课表群"""
        return cls(id=cls.DEFAULT_GROUP_GUID, name="默认", is_global=False)

    @classmethod
    def create_global_group(cls) -> "ClassPlanGroup":
        """创建全局课表群"""
        return cls(id=cls.GLOBAL_GROUP_GUID, name="全局课表群", is_global=True)
...
class ClassPlan:
    ...
    def get_valid_time_layout_items(
        ...
        # 获取课程映射
        time_layout_map: Dict[int, ClassInfo] = {
            class_info.index: class_info for class_info in self.classes
        }
    ...
    def refresh_classes_list(self, time_layout: TimeLayout) -> None:
        """刷新课程列表，使其与时间布局对齐"""
        if not time_layout:
            return
    ...
    def refresh_is_changed_class(
        self, overlay_source_plan: Optional["ClassPlan"] = None
    ) -> None:
        """刷新换课标记"""
        if not overlay_source_plan or len(self.classes) != len(
            overlay_source_plan.classes
        ):
            # 清除所有换课标记
        ...
        for i, class_info in enumerate(self.classes):
            if i < len(overlay_source_plan.classes):
                ...
                class_info.is_changed_class = (
                    class_info.subject_id != source_class_info.subject_id
                )
            else:
                class_info.is_changed_class = False
    ...
    def remove_time_point_safe(
        self, time_point: TimeLayoutItem, time_layout: TimeLayout
    ) -> None:
        """安全地删除时间点"""
        # 移除对应的课程
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overlay_setup_time": (
                self.overlay_setup_time.isoformat() if self.overlay_setup_time else None
            ),
            "is_enabled": self.is_enabled,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ClassPlan":
        ...
        plan.associated_group = data.get(
            "associated_group", ClassPlanGroup.DEFAULT_GROUP_GUID
        )
